...
def camp_join_kb(camp_id: str, count: int):
...
async def build_msg(camp_id: str, start: datetime, end: datetime):
    ...
    dua = (
        s["camp_dua"]
        or "🤲 «رَبِّ زِدْنِي عِلْمًا وَارْزُقْنِي فَهْمًا»"
    )
    ...
    remaining = max(
        0,
        int((end - datetime.now()).total_seconds())
    )
...
async def countdown_task(
    ...
    while True:
        ...
        if (
            chat_id not in active_camps
            or active_camps[chat_id]["camp_id"] != camp_id
        ):
            return
...
async def start_camp(
    ...
    sent = await message.answer(
        text,
        parse_mode="Markdown",
        reply_markup=camp_join_kb(camp_id, 0)
    )
